...
export interface NotificationData{
    ...
    name: string,
    text: string,
    onclick: (clear_notification: () => void) => void,
}

export default function NotificationSystem(){
    const [notifications, set_notifications] = useState<JSX.Element[]>([]);
    const [key, set_key] = useState(0);
    const clear_notification = (notification: JSX.Element) => set_notifications(notifications.filter((other) => {return other.key != notification.key}));;
    const new_notification = (data: NotificationData) => {
        let notification = <div className='notification' key={key} 
        onClick={() => data.onclick(() => {clear_notification(notification)})}>
